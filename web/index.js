...
const PORT = parseInt(
  process.env.BACKEND_PORT || process.env.PORT || "80",
  10
);
...
app.post("/api/offer", cors(), async (req, res) => {
  try {
    ...
    jwt.verify(req.body.token, 'f228b20d96ea23e8ed7cef1f5a6c6631');
    
    
    console.log("varify 1");
  } catch (e) {
    res.status(401).send("Unauthorized");
    console.log("varify 1" , e);
  }
...
app.post("/api/sign-changeset", cors(), async (req, res) => {
  try {
    jwt.verify(req.body.token,'f228b20d96ea23e8ed7cef1f5a6c6631');
    console.log("varify 2");
  } catch (e) {
    res.status(401).send("Unauthorized");
    console.log("varify 2" , e);
  }
  const selectedOffer = req.body.changeval;
  const payload = {
    iss: '0d3f51e5a3309561a145ccd20d258bf8',
    jti: uuidv4(),
    iat: Date.now(),
    sub: req.body.referenceId,
    changes: selectedOffer,
  };
  const token = jwt.sign(payload, 'f228b20d96ea23e8ed7cef1f5a6c6631');
...
const OFFERS = [
  {
    ...
    productImageURL:
      "https://cdn.shopify.com/s/files/1/0760/1566/5428/products/Main_0a40b01b-5021-48c1-80d1-aa8ab4876d3d.jpg?v=1683721116", // Replace this with product image's URL.
    ...
    changes: [
      {
        type: "add_variant",
        variantID: 45105814896916, // Replace with the variant ID.
        quantity: 1,
        discount: {
          value: 15,
...
function getOrderID(session, refId) {
  shopify.api.rest.Checkout.find({
    session: session,
    token: refId,//"bd5a8aa1ecd019dd3520ff791ee3a24c",
  });
}